...
TEST_CASE("The Barrier with busy wait works correctly") {
    ...
    SECTION("The last thread will call the function") {
        auto thread_fn = [&]() {  
            ...
            sync_point.busy_wait([](){
                std::cout << "I'm the last thread to reach the barrier when using busy waiting! (You should see this message exactly one time)" << std::endl;
            });
        };
