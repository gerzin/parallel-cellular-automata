...
TEST_CASE("All the C.A. compute the same result")
{
    ...
    auto g1 = ca::Grid<int>(100, 100);
    auto g2 = ca::Grid<int>(100, 100);
    auto g3 = ca::Grid<int>(100, 100);
    auto g4 = ca::Grid<int>(100, 100);
    for (int i = 0; i < 100; ++i)
    {
        g1(i, i) = i;
        g2(i, i) = i;
    ...
    auto update = [&](int c, int tl, int t, int tr, int l, int r, int bl, int b, int br) {
        auto sum = c + tl + t + tr + l + t + l + bl + b + br;
        if (sum > 2)
        {
            return 1;
        }
    ...
    auto ff = ca::ff::CellularAutomaton<int>(g4, update, nw);
    ...
    REQUIRE(seq.get_generation() == nsteps);
